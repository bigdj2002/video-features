...
class TEST_TC : public ::testing::Test
{
public:
  struct tc_params
  {
...
protected:
  std::vector<tc_params> test_cases;
  std::ifstream file;
  ...
  void SetUp() override
  {
    ...
    while (true)
    {
      std::unique_ptr<uint8_t[]> frame(new uint8_t[gop_size]);
      file.read(reinterpret_cast<char *>(frame.get()), gop_size * sizeof(uint8_t));
      ...
      std::memcpy(ref0_frame.get(), frame.get(), frame_y_size);
      params.ref0_frames.push_back(std::move(ref0_frame));

      std::unique_ptr<uint8_t[]> cur_frame(new uint8_t[frame_y_size]);
      std::memcpy(cur_frame.get(), frame.get() + frame_yuv_size, frame_y_size);
